...
int main(int argc, char* argv[]) {
	...
	enum class mode {getList, getAns, redoHomework, help};
	...
	auto redoMode = (
		clipp::command("redoHomework").set(selected,mode::redoHomework),
		clipp::required("-t", "--teacher-token") & clipp::value("tchToken", tchToken) % "ANY Teacher's Token",
	...
	auto cli = (
    (getListMode | getAnsMode | redoMode | clipp::command("help").set(selected,mode::help) ),
    ...
    switch(selected) {
		...
		case mode::redoHomework: std::cout << redoHomework(tchToken, stuId, hwId) << std::endl; break;
